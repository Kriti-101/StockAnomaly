import psycopg2
import requests, time, random
from datetime import datetime, time as dtime
import pytz

conn = psycopg2.connect("host=localhost dbname=stockdb user=postgres password=Zener$45")
cur = conn.cursor()

headers = {'User-Agent': 'Mozilla/5.0'}
ist = pytz.timezone('Asia/Kolkata')

# market hours in IST
MARKET_OPEN  = dtime(9, 15)
MARKET_CLOSE = dtime(15, 30)

last_sim_price = None  # for simulated moves

def market_is_open(now_ist: datetime) -> bool:
    t = now_ist.time()
    return MARKET_OPEN <= t <= MARKET_CLOSE

print("🚀 NSE → Postgres LIVE (every 2s)")
while True:
    try:
        now_ist = datetime.now(ist)

        r = requests.get(
            'https://www.nseindia.com/api/quote-equity?symbol=RELIANCE',
            headers=headers,
            timeout=10
        )
        data = r.json()

        price_info    = data.get('priceInfo', {})
        security_info = data.get('securityInfo', {})

        real_price  = price_info.get('lastPrice')
        real_change = price_info.get('change')
        real_vol    = security_info.get('value', 0) or data.get('totalTradedVolume', 0)

        if market_is_open(now_ist) and real_price is not None:
            # use real market data
            price  = real_price
            change = real_change
            vol    = real_vol
            source = "REAL"
        else:
            # simulate a small random walk around last known or close price
            base = real_price or last_sim_price or 1500.0
            step = random.uniform(-0.003, 0.003)  # ±0.3%
            price = round(base * (1 + step), 2)
            change = (price - (real_price or base)) if real_price else 0.0
            vol = 0
            source = "SIM"

        last_sim_price = price

        print(f"{source} PARSED:", price, vol, change)

        cur.execute(
            "INSERT INTO stock_prices (timestamp, symbol, price, volume, change) "
            "VALUES (NOW(), %s, %s, %s, %s)",
            ('RELIANCE', price, vol, change)
        )
        conn.commit()
        print(f"✅ INSERTED ({source}): {price} {vol} {change}")

    except Exception as e:
        print("❌ ERROR:", repr(e))

    time.sleep(2)